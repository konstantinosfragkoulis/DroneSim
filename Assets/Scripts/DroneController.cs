...
public class DroneController : MonoBehaviour {
    ...
    void Start() {
        ...
        // Map the shared memory object once
        addr = mmap(IntPtr.Zero, (uint)(sizeof(int) * 16), PROT_READ, MAP_SHARED, fd, 0);
        if (addr == (IntPtr)(-1)) {
    ...
    void FixedUpdate() {
        if(arm) {
            ...
            rb.rotation = Quaternion.Euler(Roll, Pitch, Yaw); // Rotation in the Z, X, and Y axes

        }
    }

    void Update() {
        // Refresh the values array from the shared memory
